...
public:
    ...
    static Packet createDataPacket(
        ...
        packet.header.seq_num = sequenceNumber;
        packet.header.flags = static_cast<uint8_t>(
            PacketFlag::DATA
        );
        packet.payload = data;
    ...
    vector<unsigned char> serialize() {
        ...
        header.checksum = calculateChecksum(bytes);

        bytes[10] = static_cast<unsigned char>(
            header.checksum >> 8
        );
        bytes[11] = static_cast<unsigned char>(
            header.checksum & 255
        );
    ...
    static Packet deserialize(
        ...
        packet.header.seq_num = readUint32(
            bytes.data()
        );
        packet.header.ack_num = readUint32(
            bytes.data() + 4
        );
        packet.header.length = readUint16(
            bytes.data() + 8
        );
        packet.header.checksum = readUint16(
            bytes.data() + 10
        );
        ...
        if (
            ...
            !=
            static_cast<size_t>(
                HEADER_SIZE + packet.header.length
            )
        ) {
            throw runtime_error(
        ...
        if (!validFlag(packet.header.flags)) {
            throw runtime_error(
...
private:
    ...
    static bool validFlag(
        ...
        return
            flag == static_cast<uint8_t>(PacketFlag::DATA)
            ||
            flag == static_cast<uint8_t>(PacketFlag::ACK)
            ||
            flag == static_cast<uint8_t>(PacketFlag::FIN);
    }
    ...
    static bool checksumValid(
        ...
        receivedChecksum = static_cast<uint16_t>(
            receivedChecksum | bytes[11]
        );
