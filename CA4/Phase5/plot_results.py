...
result_directory = Path("results") / "phase5" / test_name
plot_directory = result_directory / "plots"
cwnd_file = result_directory / "cwnd.csv"
throughput_file = result_directory / "throughput.csv"
summary_file = result_directory / "sender_summary.txt"
...
with cwnd_file.open(newline="", encoding="utf-8") as file:
...
plt.title(
    f"Congestion Window Over Time - {test_name}\n"
    f"Loss {loss_percent}%, ACK delay {ack_delay_ms} ms"
)
plt.legend()
plt.grid(True)
plt.tight_layout()
...
with throughput_file.open(newline="", encoding="utf-8") as file:
    reader = csv.DictReader(file)
...
plt.title(
    f"Throughput Over Time - {test_name}\n"
    f"Loss {loss_percent}%, ACK delay {ack_delay_ms} ms"
)
plt.legend()
plt.grid(True)
