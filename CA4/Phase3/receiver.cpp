...
const int ACK_DELAY_MS = 100;
const unsigned int RANDOM_SEED = 12345;
const string LOG_FILE = "results/phase3/logs/receiver.log";
...
bool shouldDropPacket() {
    ...
    return
        lossDistribution(randomGenerator)
        <=
        LOSS_PERCENT;
}
...
void sendAck(
    ...
    Packet ackPacket =
        Packet::createAckPacket(ackNumber);
        ...
        try {
            ...
            if (shouldDropPacket()) {
                ...
                logger.log(
                    ...
                    + " seq="
                    + to_string(packet.header.seq_num)
                );
...
int main() {
    try {
        ...
        logger.log(
            ...
            + " loss_percent="
            + to_string(LOSS_PERCENT)
            + " ack_delay_ms="
            + to_string(ACK_DELAY_MS)
            + " random_seed="
            + to_string(RANDOM_SEED)
        );
        ...
        logger.log(
            ...
            + " dropped_packets="
            + to_string(droppedPackets)
            + " delayed_acks="
            + to_string(delayedAcks)
            + " loss_percent="
            + to_string(LOSS_PERCENT)
            + " ack_delay_ms="
            + to_string(ACK_DELAY_MS)
